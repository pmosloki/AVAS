...
/* Actual period: 0.000011 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x528,
  .duty_cycle_counts = 0x294, .source_div = (timer_source_div_t) 0, .channel = 2, .p_callback = NULL,
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.000125 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x3a98,
  .duty_cycle_counts = 0x1d4c, .source_div = (timer_source_div_t) 0, .channel = 8, .p_callback = Timer_CallBack,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
